...
#else
    const char* pszModule = "BitcoinTitanium";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinTitanium
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinTitanium
    // Mac: ~/Library/Application Support/BitcoinTitanium
    // Unix: ~/.BitcoinTitanium
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinTitaniumData";
#else
    fs::path pathRet;
    ...
    return pathRet / "BitcoinTitaniumData";
#else
    // Unix
    return pathRet / ".BitcoinTitaniumData";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "BitcoinTitanium.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "BitcoinTitaniumd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong BitcoinTitanium will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("BitcoinTitanium"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
