...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("BitcoinTitanium-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BitcoinTitaniumTEST2\r");
            Send(hSocket, "WHO #BitcoinTitaniumTEST2\r");
        } else {
            // randomly join #BitcoinTitanium00-#BitcoinTitanium05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #BitcoinTitanium%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BitcoinTitanium%02d\r", channel_number).c_str());
        }
