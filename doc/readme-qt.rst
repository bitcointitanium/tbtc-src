BitcoinTitanium-qt: Qt4 GUI for BitcoinTitanium
===============================
...
Alternatively, install Qt Creator and open the `BitcoinTitanium-qt.pro` file.

An executable named `BitcoinTitanium-qt` will be built.
...
A warning for people using the *static binary* version of BitcoinTitanium on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of BitcoinTitanium is linked against libdb5.3.
...
installed causes BitcoinTitanium-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
